...
\newpage
\begin{abstract}
  Quantifying forest ecosystem change informs us about climate and
  earth systems interactions. Fossil pollen data provides us with
  information about past forest ecosystem change, but its
  interpretation requires an understanding of the pollen-vegetation
  relationship. The pollen-vegetation relationship is complex,
  depending on differential production and dispersal, which in turn
  depend on a range of factors. There is concern that widespread
  land-use change may have altered this relationship. Here we use a
  Bayesian hierarchical spatial model to quantify the
  pollen-vegetation relationship, and calibrate this model using a
  newly developed settlement-era calibration data set. This data set
  consists of Public Land Survey (PLS) forest composition data, and
  fossil pollen records identified as being from the settlement-era
  through expert elicitation. Production and dispersal results from
  the calibration model are consistent with our previous understanding
  of these processes, although source area estimates are
  larger. Settlement-era vegetation composition predictions based on
  the calibration model demonstrate the ability of the model to
  prediction regional compositional patterns.
\end{abstract}
...
about prehistoric forest composition. Rigor- ous estimates of past
forest composition from these records rely on our ability to quantify
the relationship between the composition of fossil pollen assemblages
and the composition of the vegetation that produced them.
...
%% approach is the ability to borrow information across space to estimate
%% governing parameters and their uncertainty at a regional scale,
%% without the reliance on prior quantification of ecological or physical
%% processes.
...
%XXX lit on larger source areas???

...
for the Upper Midwest. We then use these results to predict
settlement-era vegetation composition.
...
in the domain is computationally intractable, therefore the underlying
gaussian process is estimated using a gaussian predictive process
...
using the settlement-era sediment pollen calibration data set for a
suite of underlying candidate calibration models. Note that we omit
the Lower Peninsula from this subdomain because it is disjoint and
distant, and as such we do not expect significant contributions to
deposited pollen in our subdomain to originate from there.

The dissimilarity between the predicted composition for different
underlying calibration models and the PLS composition estimates is
determined by measuring the Euclidean distance between the two sets of
multi-category outcomes at each grid cell. 

More specifically, we compare the settlement-era PLS composition
estimates $\bm{r}(s(i))$ with the posterior mean of the composition
estimates obtained from the prediction model
$\bm{\hat{r}}(s(i))$. Then, for location $s(i)$ the dissimilarity is
\begin{align*}
...
Dissimilarity maps for each of the candidate models illustrate that it
is difficult to distinguish between the competing predictions
(Figure~\ref{fig:maps_l2}). These maps also highlight the increased
difficulty with resolving composition in areas that compositionally
unique relative to the greater regional patterns such as the Big Woods
in Minnesota.

Total dissimilarity scores were computed for each set of predictions
(Table~\ref{table:GOF}). According to the Brier score, the
...
(Full Brier = 62.00), while the prediction model with underlying
variable Gaussian calibration model are the farthest (Full Brier =
68.82).
